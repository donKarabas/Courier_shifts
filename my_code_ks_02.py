...
class Main:
    ...
    def month_tab(self):
        try:
            ...
            address = self
            return month, address
        except AttributeError:
            tk.messagebox.showerror('Ошибка открытия!', 'Для редакции необходимо открыть таблицу '
                                                        'и выделить нужную строку.')
    ...
    def validate(self, month, ws, delivery, gw, dr, comments):
        ...
        result_entry_gw = re.match('^\d{0,2}?$', gw)
